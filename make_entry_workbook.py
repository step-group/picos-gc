...
def build(
    tubes: set[str], fresh: set[str] = FRESH_KF, aq_only: set[str] = frozenset()
) -> openpyxl.Workbook:
    ...
    for tube in sorted(tubes, key=_order):
        ...
        # an aqueous-only tube keeps campaign 1's organic phase: no row, nothing patched
        vials = [v for v in VIALS if tube not in aq_only or v[0] != org]
...
def main() -> None:
    ...
    fresh = split_aq(fresh)[0]
    out.parent.mkdir(exist_ok=True)
    wb = build(tubes, fresh, aq_only)
    ...
    wb.save(out)
    n_vials = sum(2 if t in aq_only else 4 for t in tubes)
    print(f"{out}: {len(tubes)} tubes, {n_vials} vials ({', '.join(sorted(tubes, key=_order))})")
